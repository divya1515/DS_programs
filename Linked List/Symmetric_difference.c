...
void Symmetric_difference(struct node **start1, struct node **start2,struct node **start3)
{
   struct node *p;
   struct node *q;
   p=*start1;
...
int main()
{
   ...
   struct node *start3;
   start3=NULL;
   Symmetric_difference(&start, &start1,&start3);
   printf("Symmetric difference linked list: ");
   traverse(start3);
}
